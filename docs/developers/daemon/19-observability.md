...
| `DaemonLogger` 结构化文件日志（interface）  | `serve/daemonLogger.ts`                         | `DaemonLogger` 接口定义了 `info`/`warn`/`error` 方法；结构化 JSON-like 日志行写入文件（启动时打印路径 `daemon log -> <path>`）；上下文含 `route`、`sessionId`、`clientId`、`childPid`、`channelId` 等结构化字段                                      |
